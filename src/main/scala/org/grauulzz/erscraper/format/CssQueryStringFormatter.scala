...
case class CssQueryStringWithHtmlTagList(query: String, htmlTagList: List[HtmlTag]) extends QueryString
...
object CssQueryStringFormatter {
  ...
  def format(queryString: QueryString): String = queryString match {
    ...
    case CssQueryStringWithHtmlTag(query, htmlTag) => val str: String = prefix(query)
      s"$str ${HtmlTag.apply(htmlTag)}"

    case CssQueryStringWithHtmlTagList(query, htmlTagList) => val str: String = prefix(query)
